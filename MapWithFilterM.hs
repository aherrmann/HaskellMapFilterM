...
import qualified SetBase as Set
import StrictFold
import StrictPair
...
-- | /O(n)/. Filter all values that satisfy the monadic predicate.

filterM :: Monad m => (a -> m Bool) -> Map k a -> m (Map k a)
filterM p m
  = filterWithKeyM (\_ x -> p x) m

-- | /O(n)/. Filter all keys\/values that satisfy the monadic predicate.

filterWithKeyM :: Monad m => (k -> a -> m Bool) -> Map k a -> m (Map k a)
filterWithKeyM _ Tip = return Tip
filterWithKeyM p (Bin _ kx x l r) = do
    keep <- p kx x
    left <- filterWithKeyM p l
    right <- filterWithKeyM p r
    if keep
    then return $ link kx x left right
